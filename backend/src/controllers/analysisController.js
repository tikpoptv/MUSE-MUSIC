...
const newAnalysis = async (req, res) => {
  try {
    ...
    logger.info('Starting new analysis (skipping existing check)', {
      userId,
      hasTranslate: actions.translate,
      hasMood: actions.mood,
      targetLanguage: translationConfig?.targetLanguage,
    ...
    const result = await AnalysisService.processNew(
      lyricsRecord,
      actions,
      translationConfig || {},
      userId,
    ...
    return res.json(
      successResponse('New analysis completed successfully', result)
    );
    
  } catch (error) {
    logger.error('Error in newAnalysis:', error);
    ...
    return res.status(500).json(
      errorResponse('Failed to process new analysis', 500, error.message)
    );
  }
};
