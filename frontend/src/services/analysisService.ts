...
export const analysisService = {
  ...
  async newAnalysis(request: AnalysisRequest): Promise<AnalysisResult> {
    const url = '/api/analysis/new';
    const res = await apiService.post<{ success: boolean; message?: string; data: AnalysisResult; statusCode?: number }>(url, request);
    
    if (!res.success || !res.data) {
      throw new Error(res.message || res.error || 'Failed to start new analysis');
    }
