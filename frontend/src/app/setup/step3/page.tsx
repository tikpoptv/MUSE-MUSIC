...
export default function SetupStep2() {
  ...
  const handleNext = async () => {
    ...
    try {
      await setupService.saveSetupStep('step3', { birthday: selectedDate.toLocaleDateString('en-CA') });
      
      const userData = localStorage.getItem('user_data');
      if (userData) {
        const user = JSON.parse(userData);
        user.stepData = user.stepData || {};
        user.stepData.step3 = { birthday: selectedDate.toLocaleDateString('en-CA') };
